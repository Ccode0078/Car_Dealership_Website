...
def index(request):
    """
    Homepage (the hero + featured cars).
    """
...
def car_details(request, id):
    ...
    URL: /car_details/<id>/
    """
    car = get_object_or_404(Listing, pk=id)
    images = Image.objects.filter(listing=car).order_by("id")
    context = {
        "car": car,
        "images": images,
    }
...
# (Optional) Backward-compat: if any old links still hit /listing/<id>/, forward them.
def listing(request, id):
    """
    Legacy view (if old templates linked to /listing/<id>/).
    Redirect to canonical detail route.
    """
    return redirect("car_details", id=id)

def create_listing(request):
    """
    Create a new listing and any uploaded images.
    Redirect to the new car's detail page on success.
    """
    if request.method == "POST":
        ...
        if listing_form.is_valid():
            ...
            # If your form includes multiple files under 'images'
            for file in request.FILES.getlist("images"):
                try:
                    Image.objects.create(listing=listing, image=file)
                except Exception as e:
                    # You could add logging here
                    print(f"Error saving file {getattr(file, 'name', 'unknown')}: {e}")

            return redirect("car_details", id=listing.id)
        # fall through on invalid form -> re-render with errors
    else:
        listing_form = ListingForm()

    return render(request, "create_listing.html", {"listing_form": listing_form})
